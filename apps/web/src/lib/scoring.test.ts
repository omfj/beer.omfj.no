...
describe('calculateDrinkPoints', () => {
	...
	it('calculates cocktail points with balanced ABV', () => {
		// Balanced cocktail ABV (10%) - reduced from 15% for fair scoring
		expect(calculateDrinkPoints(200, 10)).toBe(1.6); // Small cocktail
		expect(calculateDrinkPoints(300, 10)).toBe(2.4); // Standard cocktail
	});

	it('verifies old cocktail ABV was too high', () => {
		// Old cocktail ABV (15%) scored too high compared to beer
